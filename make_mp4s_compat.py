...
Usa el header.png (banner de marca aprobado) superpuesto arriba + crossfade.
"""
import os, subprocess

IMG_DIR = "/opt/data/cafe-veracruzano-menu-tv/img"
HEADER = os.path.join(IMG_DIR, "header.png")
OUT_DIR = "/opt/data/cafe-veracruzano-menu-tv/videos_compat"
...
def build_mp4(files, out, label):
    ...
    # header.png escalado al ancho del video (720p)
    filt += (f"[{n}:v]scale={W}:-1,format=rgba[hd];")
    filt += f"[base][hd]overlay=0:0[out0];"
    # oscurecer un poco la foto bajo el header para legibilidad (opcional, no)
    filt += f"[out0]format=yuv420p[vout]"
    inputs += ['-i', HEADER]
    cmd = ['ffmpeg','-y'] + inputs + [
...
build_mp4(POSTRES, os.path.join(OUT_DIR,"CafeVeracruzano_POSTRES_CAFES_TAMALES.mp4"), "Postres/Cafes/Tamales")
build_mp4(COCINA, os.path.join(OUT_DIR,"CafeVeracruzano_MENU_COMPLETO.mp4"), "Menu completo (cocina)")
print("\nDONE")